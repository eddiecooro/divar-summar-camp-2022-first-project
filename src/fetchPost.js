...
if (url.includes("post.html")) {
  ...
  fetch(`http://localhost:9000/api/v8/posts-v2/web/${category}`, {
    ...
    .then((b) => {
      ...
      let infobox = null
      try {
        infobox = deepSearchByPair(data, "widget_type", "GROUP_INFO_ROW")[0]["data"]["items"]
      } catch (error) {
      }

      let rows = deepSearchByPair(data, "widget_type","UNEXPANDABLE_ROW")
      let description = deepSearchByPair(data, "widget_type","DESCRIPTION_ROW")[0]["data"]["text"]
      ...
      <main class="single-post">
        <nav class="single-post__breadcrumb">
          <span class="single-post__arrowed-text">
            املاک <img class="single-post__arrow"src="${require("./icons/svgs/chevron-left.svg" ).default}"/>
          </span>
          <span class="single-post__arrowed-text">
            فروش مسکونی <img class="single-post__arrow"src="${require("./icons/svgs/chevron-left.svg" ).default}"/>
          </span>
          <span class="single-post__arrowed-text">
            آپارتمان <img class="single-post__arrow"src="${require("./icons/svgs/chevron-left.svg" ).default}"/>
          </span>
        </nav>
        ...
        <section class="single-post__right-box">
          ...
          <section class="single-post__actions">
            ...
            <button class="single-post__action-share button button--icon">
              <img src="${require( "./icons/svgs/share-variant-o.svg").default}"/>
            </button>
          </section>
          ${infobox!==null?`<section class="single-post__info-box-container">
          ${infobox.map((item)=>`
          ...
          <hr class="single-post__info-row-separator"/>`).join(" ")}
          </section>
          ...
          ${chips.map((chip) => `<button class="single-post__tag">${chip.text}</button>`).join(" ")}
